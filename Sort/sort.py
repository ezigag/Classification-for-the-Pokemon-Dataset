...
def decesiontree_CrossVacation(feature,target) :
    sum = 0
    round = 1
    ...
    for train_index, test_index in kf.split(feature):
        ...
        # print('\n')
        round += 1
    print("\nAverage accuracy of testset by DecestionTree : ", sum / 10)
    return clf
...
clf = decesiontree_CrossVacation(X,y)
...
with open('tree_DT.txt', 'w') as f:
   tree.export_graphviz(clf, out_file=f,
    ...
    class_names=list(np.array(df['type1'].drop_duplicates().reset_index(drop=True))),
    filled=True)
